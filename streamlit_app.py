...
def gerar_grafico_html_json(link, nome_estacao, cota_aten, cota_alerta, cota_inundacao):
    try:
        ...
        ultimo_nivel = df['Nivel'].iloc[-1] * 100  # em cm
        ...
        plt.plot(df['DataHora'], df['Nivel'], color='#88CDF6', linewidth=2, label='Nível do rio')
        if pd.notna(cota_aten): plt.axhline(cota_aten/100, color='gold', linestyle='--', label='Cota de Atenção')
        if pd.notna(cota_alerta): plt.axhline(cota_alerta/100, color='orange', linestyle='--', label='Cota de Alerta')
        if pd.notna(cota_inundacao): plt.axhline(cota_inundacao/100, color='red', linestyle='--', label='Cota de Inundação')
...
def criar_mapa_completo(df):
    ...
    for idx, row in df.iterrows():
        link = row['Link_graf']
        popup_html = ""
        categoria = row['Ícone'] if pd.notna(row['Ícone']) else None

        if link.endswith('.json'):
            popup_html, categoria_calc = gerar_grafico_html_json(
                link,
                row['Nome'],
                row['Cota de Atenção (cm)'],
                row['Cota de Alerta (cm)'],
                row['Cota de Inundação (cm)']
            )
            if categoria is None:
                categoria = categoria_calc
                df.at[idx, 'Ícone'] = categoria
        else:
            dados = extrair_dados_sgb(link)
            if dados is not None and not dados.empty:
                fig, ax = plt.subplots()
                ax.plot(dados['timestamp'], dados['nivel'], color='#88CDF6', linewidth=2, label='Nível do rio')
                if pd.notna(row['Cota de Atenção (cm)']): ax.axhline(row['Cota de Atenção (cm)'], color='gold', linestyle='--', label='Cota de Atenção')
                if pd.notna(row['Cota de Alerta (cm)']): ax.axhline(row['Cota de Alerta (cm)'], color='orange', linestyle='--', label='Cota de Alerta')
                if pd.notna(row['Cota de Inundação (cm)']): ax.axhline(row['Cota de Inundação (cm)'], color='red', linestyle='--', label='Cota de Inundação')
                ax.legend()
                fig.autofmt_xdate()
                buf = io.BytesIO()
                ...
                categoria = categoria or 'SemTransmisso'
                df.at[idx, 'Ícone'] = categoria

        cor = icone_cores.get(categoria, 'blue')
        popup = folium.Popup(IFrame(popup_html, width=470, height=370), max_width=470)
